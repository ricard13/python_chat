...
def receber():                                                                          # Função que vai receber as conexões dos clientes
    while True:
        ...
        t2 = threading.Thread(name="handle", target=handle, args=(cliente, endereco,))  # Criação e execução da thread da função handle()
        t2.start()                                                                      # Iniciação da thread handle()
        time.sleep(0.2)                                                                 # Delay da thread


t = threading.Thread(name="receber", target=receber)                                    # Criação da thread receber() para receber os dados ao mesmo tempo que envia
t.start()                                                                               # Iniciação da thread receber()
